# Performance Shakedown Command Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Create a `/perf-shakedown` slash command that performs automated performance analysis across all ECS hot paths using 4 parallel subagents.

**Architecture:** A single `.claude/commands/perf-shakedown.md` file contains the orchestrator prompt. The orchestrator determines scope (diff-based or full), dispatches 4 parallel subagents (data-layout, vectorization, rkyv-compat, cache-and-misc), aggregates their reports, and appends profiling recommendations. No Rust code — this is a pure prompt engineering task.

**Tech Stack:** Claude Code slash commands (YAML frontmatter + markdown prompt), Agent tool for subagent dispatch, Bash/Grep/Glob/Read for analysis.

---

### Task 1: Create the command file with frontmatter and Phase 1 scoping logic

**Files:**
- Create: `.claude/commands/perf-shakedown.md`

**Step 1: Create the command file**

Write `.claude/commands/perf-shakedown.md` with the following content. This is the complete orchestrator prompt.
...
args:
  - name: target
    description: "Scope: 'all' for full codebase, file path/module filter, or omit for diff-based (default on feature branches)"
    required: false
allowed-tools: Bash, Read, Glob, Grep, Agent
---
```

The body must contain these sections in order. The exact prompt text follows — write it verbatim.

**Section 1: Introduction paragraph**

```markdown
Run a performance shakedown on the Minkowski ECS codebase. This is a three-phase analysis:
Phase 1 determines scope, Phase 2 dispatches 4 parallel subagents for deep analysis,
Phase 3 synthesizes profiling recommendations.
```

**Section 2: Phase 1 — Scope Resolution**
...
3. Otherwise (no arguments on a feature branch): run `git diff main...HEAD --name-only` and intersect with the hot path list.
...
**Section 3: Static Hot Path List**

This is the hardcoded reference embedded in the prompt. Write it as a markdown section:
...
**Section 4: Phase 2 — Subagent Dispatch**

This section instructs the orchestrator to launch 4 agents in parallel. Each agent prompt is embedded inline.

```markdown
## Phase 2 — Analysis

Read the scoped files identified in Phase 1. Then dispatch **all 4 agents in a single message** (parallel execution) using the Agent tool. Pass each agent the list of scoped files and the specific functions to examine.
...
**Section 5: Phase 3 — Profiling Recommendations**

```markdown
## Phase 3 — Profiling Recommendations

After all 4 agents complete, aggregate their reports into the output format below. Then append:

### Benchmark Coverage

1. List all `benches/` files: `ls crates/minkowski/benches/`
...
2. Flag these as "consider adding to the hot path list in this command".
3. Suggest `cargo flamegraph` or `perf record` commands for examples that exercise the changed code. Reference the examples table:
...
[Benchmark coverage gaps, suggested commands, new hot path candidates]
...
Run: `ls -la .claude/commands/perf-shakedown.md`
Expected: file exists with non-zero size

**Step 3: Validate YAML frontmatter**

Run: `head -8 .claude/commands/perf-shakedown.md`
Expected: valid YAML between `---` delimiters with `description`, `args`, and `allowed-tools` fields

**Step 4: Commit**

```bash
git add .claude/commands/perf-shakedown.md
git commit -m "feat: add /perf-shakedown performance analysis command"
```

---

### Task 2: Smoke-test the command on current codebase

This is a manual verification step — no code changes.

**Step 1: Run the command in full-codebase mode**

Run: `/perf-shakedown all`

Watch for:
- Phase 1 correctly identifies all hot path files
- 4 agents dispatch in parallel (single message with 4 Agent tool calls)
- Each agent returns findings with severity ratings and file:line references
- Phase 3 produces benchmark coverage analysis and profiling suggestions
- Output follows the structured report format

**Step 2: Verify diff-scoped mode**

Create a test branch, touch a hot path file, and run without arguments:

```bash
git checkout -b test/perf-shakedown-smoke
echo "// test" >> crates/minkowski/src/storage/blob_vec.rs
```

Run: `/perf-shakedown`

Watch for:
- Phase 1 detects only `blob_vec.rs` in the diff
- Agents receive only `blob_vec.rs` (and related files) in their scope
- Report is narrower than the full-codebase run

**Step 3: Clean up test branch**

```bash
git checkout crates/minkowski/src/storage/blob_vec.rs
git checkout main
git branch -D test/perf-shakedown-smoke
```

**Step 4: If any issues found, fix the command and amend the commit**

If the smoke test reveals prompt issues (agents not receiving correct scope, wrong output format, etc.), edit `.claude/commands/perf-shakedown.md` and commit the fix.
...
**Files:**
- Modify: `CLAUDE.md` (add command to Build & Test Commands or a new section if appropriate)
- Modify: `.claude/skills/minkowski-guide.md` (add command reference if a commands section exists)

**Step 1: Check if CLAUDE.md lists commands**

Run: `grep -n "perf\|shakedown\|self-audit\|soundness-audit" CLAUDE.md`

If other audit commands are listed, add `/perf-shakedown` alongside them. If not, skip this file — commands are self-discoverable.

**Step 2: Check if minkowski-guide.md references commands**

Run: `grep -n "command\|audit\|slash" .claude/skills/minkowski-guide.md | head -20`

If there's a commands section, add the new command. Otherwise skip.

**Step 3: Commit documentation updates (if any)**

```bash
git add -A
git commit -m "docs: reference /perf-shakedown in project docs"
```
