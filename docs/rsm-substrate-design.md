...
|---|---|---|
| Leader `Replicated<S>` | `Durable<S>`, WAL read access, registry keyed by follower identity and session generation | Delegate transactions unchanged; serve ranges; track reported settled boundaries for retention |
...
| 4.1 VSR normal delivery | leader-side per-follower cursor, send transport, opaque `Ack` | Reuse the same framing and ingest contract |
