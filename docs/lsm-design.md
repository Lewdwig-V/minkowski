...
| Hybrid dense storage | `RawCopy` is an explicit unsafe certification — [`RawCopyCertified`][rcc]: the native byte image equals the archived byte image **for every value** (no custom `Archive` impl or `rkyv(with = ...)` transform), and `T` owns no heap resource (`Copy`). Licenses both memcpy directions: flush/recovery (native bytes verbatim) and WAL replay (`decode`'s memcpy branch imports archived bytes as native). `Serialized` (rkyv per row) is the default for everything else. Kind recorded per column in the run schema | `register_raw_copy_as` bound; `impl_raw_copy_certified!`; `box_component_is_not_raw_copyable` |
...
| Replay floor | WAL tail replays records with `seq >= seq_hi` of the newest run — inclusive, so a removal that straddles the flush boundary is not lost. Acknowledged checkpoints may run ahead of the baseline (nothing was dirty — e.g. a tail-only despawn); the floor never moves with them, and the WAL is never truncated below the floor | `recover.rs` replay-floor invariant comment; `tail_only_despawn_checkpoint_does_not_resurrect`; `sparse_only_removal_checkpoint_does_not_resurrect` |
| Dirty-page discipline | row-level `BlobVec` writes mark their page dirty; batch entry points that hand out raw or mutable row access (`World::query` with `&mut T`, `query_table_mut`, `query_table_raw`, and the planner's `execute_stream_batched`/`execute_stream_join_chunk` for mutable queries) mark whole columns dirty, because per-row attribution is impossible ahead of iteration; flush writes the dirty dense/entity pages, plus allocator metadata and the complete sparse state unconditionally | `storage::dirty_pages` wired into `BlobVec` write paths; `mark_all_pages_dirty` at the batch entry points; `batch_mutation_entry_points_mark_pages_dirty` |
