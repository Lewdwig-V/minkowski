...
**Recovery is import-driven and raw-copy-bound** (the invariant that governs flush/recover correctness):
- **Raw-copyable only.** A dense archetype column is flushed as the component's *native* in-memory bytes (`World::column_page_bytes`) and copied back verbatim on recovery, sound only for plain-old-data (no heap pointers — they dangle across a restart). Enforced at two gates so it can never be violated silently: `CodecRegistry::register` rejects a non-raw-copyable type (`CodecError::NotRawCopyable`, i.e. `size_of::<T>() != size_of::<T::Archived>()`; ZSTs pass), and `FlushWriter` hard-errors on any *dense* component that lacks a codec. Sparse components instead round-trip through rkyv (`serialize_sparse`/`insert_sparse_raw`), so they need not be raw-copyable in the same way but still require a registered codec (skip-with-warning if absent).
- **Bulk column-granular reconstruction.** `materialize_world` rebuilds each archetype via the unsafe `World::import_target`/`import_page` seam — one memcpy per (archetype, component) page through `BlobVec::append_bytes_unchecked` — not row-by-row. The `unsafe` precondition (bytes are a valid native image of a raw-copyable type) is discharged by the two gates above plus per-page CRC validation on read.
