...
- **Hybrid dense storage kind.** Dense component persistence is *hybrid*, derived from the codec and recorded per column in the run schema (`StorageKind::{RawCopy, Serialized}`):
  - **RawCopy** — compile-time opt-in via `register_raw_copy_as::<T>` requiring `T: Copy` (no `Drop`, hence no owned heap resource — the only way verbatim native-byte round-trips are sound; size equality alone is unsound, e.g. `Box<u64>`, and `rkyv::Portable` does not exclude it) **and** `size_of::<T>() == size_of::<T::Archived>()` (ZSTs pass). Flushed as *native* in-memory bytes (`World::column_page_bytes`), zero-padded to `PAGE_SIZE * item_size`, copied back verbatim on recovery. Default `register_as` classifies everything `Serialized`.
  - **Serialized** — a heap-backed component (`String`/`Vec`/…) is rkyv-serialized per row into a variable-length `serialized_page` body (offset table + values, no padding) on flush, and decoded row-by-row back into a contiguous native buffer on recovery (`native_column_page`). `BlobRef` persists as a Serialized column.
  Every dense component still requires a registered codec, enforced so it can never be violated silently: `CodecRegistry::register` requires a codec for every dense type and `FlushWriter` hard-errors on any *dense* component that lacks one. Sparse components round-trip through rkyv (`serialize_sparse`/`insert_sparse_raw`) and likewise require a registered codec (skip-with-warning if absent).
- **Bulk column-granular reconstruction.** `materialize_world` rebuilds each archetype via the unsafe `World::import_target`/`import_page` seam — one memcpy per (archetype, component) page through `BlobVec::append_bytes_unchecked` — not row-by-row. Recovery first normalizes each stored page to native bytes once (`native_column_page`: RawCopy = the stored bytes, Serialized = per-row rkyv decode), then slices by native stride, so the `unsafe` precondition (bytes are a valid native image of the component type) holds for both kinds. It is discharged by the codec gate above plus per-page CRC validation on read.
- **Identity is by TYPE, never by numeric `ComponentId`.** `ComponentId` is a *per-world* index. The on-disk schema stores each component's Rust `type_name`; recovery resolves it against the *recovered* world's own component table (`resolve_local_component`), and the flush gate validates a codec by `TypeId` (`CodecRegistry::has_codec_for_type`, `World::component_type_id`), never by matching ids across registry/world. This is what makes columns reassemble correctly under non-uniform archetypes (the codex #4 fix: key columns by name, not signature position) and lets a world and its codec registry that assigned a type different ids — e.g. after recovery re-registers types into a fresh world — still round-trip. A `WorldId` binding was rejected because `recover_world` builds a fresh world, which must remain re-flushable with the same codecs.
- **Sparse flush resolves by type too.** Both dense and sparse flushing resolve the codec by the world's component `TypeId`, never by its per-world `ComponentId`. Sparse flush (`FlushWriter`, `world.sparse_component_ids()` loop) calls `CodecRegistry::serialize_sparse_by_type(world, comp_id)`, which looks up the codec by `world.component_type_id(comp_id)` and returns the codec's stable name for the blob. The `serialize_sparse_fn` closure serializes `T` directly (it is monomorphized for `T`), so the world's sparse id is used only to iterate `iter_sparse`, not to select a codec. This means reflushing a recovered world (whose re-registered ids are compacted, diverging from the registry) preserves sparse data instead of silently dropping it. WAL replay is the other id-translating path; it remaps recorded ids to local ids via `build_remap` (unmapped = hard error).
...
- **Invariant matrix rule**: an invariant that spans multiple consumers (paths, modules, on-disk frame kinds) gets a matrix in the design document before implementation — consumers as rows, input kinds as columns, each cell marked "enforced + test name" or "exempt + reason". Implementation fills no cell during code review; every cell is decided up front. Past failures: the view fence was specified as "stale-view frames are dropped", but its real surface was replay × cursor-seek × cursor-batch × segment-advance × open-scan, against Schema/Mutations/Checkpoint frames — three review rounds found the empty cells one at a time (checkpoint arm, seek loop, segment advance, cross-segment resume, active tail, schema exception).
- **Chokepoint rule** (extension of the bypass-path invariant rule): an invariant shared by N paths lives at the single convergence point all paths route through, and new paths route through it — never re-implement. The tick guard lives in `apply_record`, not in `replay_from`/`apply_batch`/`Follower::advance`; the view fence arithmetic lives in shared scans, not per consumer. If a new path re-implements an invariant, that is a bug even if the re-implementation is correct today.
- **Convention sheet rule**: numeric conventions (sequence bases, inclusive vs exclusive bounds, high-water vs last-applied semantics) are written down once with exact names in the design document, every consumer cites them, and one test pins each boundary edge. Past failures: 0-based WAL sequences vs a high-water initialized to 0 skipped record 0; the recovery replay floor (`seq_hi`, inclusive) vs a follower high-water that gap-poisoned on the first shipped record; baseline coverage (`< baseline_seq`) vs gap detection.
- **Contract verification rule** (extension of verify-before-design): a design document may only claim primitive behaviors it has verified — atomicity, idempotency, return-on-duplicate, rollback. Cite the code path or test that proves each claim. Past failures: the 4.0 spec claimed `apply_batch` was per-record atomic and `AlreadyPlaced` made re-apply idempotent; the code is non-atomic across records and `AlreadyPlaced` is a refusable error — both P1 findings.
