...
- **Hybrid dense storage kind.** Dense component persistence is *hybrid*, derived from the codec and recorded per column in the run schema (`StorageKind::{RawCopy, Serialized}`):
  - **RawCopy** — a component whose rkyv archived size equals its native size (`size_of::<T>() == size_of::<T::Archived>()`; ZSTs pass) is flushed as its *native* in-memory bytes (`World::column_page_bytes`), zero-padded to `PAGE_SIZE * item_size`, and copied back verbatim on recovery (the POD fast path).
