...
- **Known asymmetry — sparse flush is still id-keyed.** The by-`TypeId` resolution above covers *dense* archetype columns only. Sparse flushing (`FlushWriter`, `world.sparse_component_ids()` loop) still gates on `codecs.has_codec(comp_id)` and serializes via `serialize_sparse(comp_id, …)` / `stable_name(comp_id)` — all keyed by the *world's* `ComponentId`. If a registry's ids diverge from the flushed world's (e.g. reflushing a recovered world with the original registry), a sparse component is skipped with only a warning (silent baseline loss) or, if a different type occupies that id, serialized with the wrong codec. Until this is brought in line with the dense path (resolve sparse codecs by type too), persisting sparse data requires the codec registry to be registered against the world being flushed.
