...
- **Hybrid dense storage kind.** Dense component persistence is *hybrid*, derived from the codec and recorded per column in the run schema (`StorageKind::{RawCopy, Serialized}`):
  - **RawCopy** — compile-time opt-in via `register_raw_copy_as::<T>` requiring `unsafe RawCopyCertified`: the native byte image equals the archived byte image **for every value** (no custom `Archive` impl or `rkyv(with = ...)` transform — size equality and `Copy` alone do not exclude them) and `T` owns no heap resource. Licenses both memcpy directions: native→archived on recovery, archived→native in `decode`'s WAL-replay branch. Default `register_as` classifies everything `Serialized`.
