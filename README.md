...
let path = flush_and_record(&world, seq_range, &mut manifest, &mut log, &dir, &codecs)?;
...
**Dense component persistence is hybrid.** A component whose rkyv archived size equals its native size is stored **RawCopy** (native bytes, bulk-memcpy recovery — the POD fast path); a heap-backed component (`String`/`Vec`/…) is stored **Serialized** (rkyv, variable-length page, per-row decode). The kind is derived from the codec and recorded per column in the run schema, so a RawCopy column round-trips by memcpy while a Serialized column is rkyv-encoded on flush and decoded row-by-row on recovery. `BlobRef` persists as a Serialized column. Every dense component still requires a registered codec, enforced so it can never be violated silently: `CodecRegistry::register` requires a codec for every dense type and the flush path hard-errors on any dense component lacking one. Recovery reconstructs by **type**, never by numeric id: `ComponentId` is a per-world index, so the on-disk schema stores the component's type name and recovery resolves it against the recovered world's own component table — and the Serialized codec is always resolved by `TypeId` (`World::component_type_id`), never `ComponentId`. Both dense archetype columns and sparse components resolve their codec by type, so a world and its codec registry that assigned a type different ids — e.g. after recovery re-registers components into a fresh world with compacted ids — still round-trip correctly.
