...
**Persisted components must be raw-copyable.** A dense archetype column is flushed as the component's native in-memory bytes and copied back verbatim on recovery, which is sound only for plain-old-data types (no `String`/`Vec`/other heap pointers, which would dangle across a restart). The engine enforces this in two places so it can never be violated silently: `CodecRegistry::register` rejects a non-raw-copyable type (`CodecError::NotRawCopyable`, detected as `size_of::<T>() != size_of::<T::Archived>()`), and the flush path refuses any dense component lacking a registered codec. Recovery reconstructs by **type**, not by numeric id: `ComponentId` is a per-world index, so the on-disk schema stores the component's type name and recovery resolves it against the recovered world's own component table. This makes **dense archetype columns** round-trip even when a world and its codec registry assigned a type different ids (e.g. after recovery re-registers into a fresh world). **Sparse components are the current exception**: the flush path looks up their codec by the world's `ComponentId`, so persisting sparse data requires the codec registry to be registered against the world being flushed — re-register codecs against a recovered world before flushing its sparse components, or they will be skipped with a warning.
