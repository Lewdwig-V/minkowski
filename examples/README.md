...
| `persist` | Full persistence lifecycle with 100 entities across 3 archetypes. LSM flush baseline, `Durable`-wrapped `QueryWriter` reducer (WAL-backed), crash recovery via `recover_world` + persistent index catch-up. |
| `replicate` | Pull-based WAL replication via channel-simulated network. Source sends LSM sorted-run bytes + WAL batch bytes; replica reconstructs independently with its own `CodecRegistry` (different registration order). Verifies convergence. |
